...
function RiftToken:get_value()
    ...
    if not (self.validation_bits & rift.TOKEN_BITS.INITIALIZED) ~= 0 then
...
function RiftToken:set_value(val)
    ...
    self._value = val
    self.validation_bits = self.validation_bits | rift.TOKEN_BITS.INITIALIZED
end
...
function RiftToken:lock()
    ...
    if not self._lock then
        ...
        self.validation_bits = self.validation_bits | rift.TOKEN_BITS.LOCKED
        return true
    end
    return false
end
...
function RiftToken:validate()
    ...
    -- Check ALLOCATED bit
    if not (self.validation_bits & rift.TOKEN_BITS.ALLOCATED) ~= 0 then
        return false
    end
    ...
    -- Mark as governed
    self.validation_bits = self.validation_bits | rift.TOKEN_BITS.GOVERNED
    return true
end
...
function RiftToken:superpose(states, amplitudes)
    ...
    self.validation_bits = self.validation_bits | rift.TOKEN_BITS.SUPERPOSED
    return true
end
...
function RiftToken:entangle_with(other, entanglement_id)
    ...
    self.validation_bits = self.validation_bits | rift.TOKEN_BITS.ENTANGLED
    return true
end
...
function RiftToken:collapse(selected_index)
    ...
    if not (self.validation_bits & rift.TOKEN_BITS.SUPERPOSED) ~= 0 then
        return false
    end
...
function RiftToken:is_valid()
    ...
    return (self.validation_bits & rift.TOKEN_BITS.INITIALIZED) ~= 0 and
           (self.validation_bits & rift.TOKEN_BITS.GOVERNED) ~= 0
end
